...
class PaymentGatewayApi
{
    ...
    use Paypal, Stripe, Manual, SslcommerzTrait, RazorTrait, FlutterwaveTrait, UddoktaPay;
    ...
    public function chargeCalculate($currency, $receiver_currency = null)
    {
        ...
        if ($receiver_currency) {
            ...
            if ($this->output['type'] == 'ORDER') {
                $trx_charge = TransactionSetting::where('slug', 'topup')->first();
                $fix_charge =  $trx_charge->fixed_charge * $sender_currency_rate;
                $top_up_game_percent_charge = ($trx_charge->percent_charge * $amount) / 100;
                $total_charge = $fix_charge + $top_up_game_percent_charge;
                $total_amount = ($amount * $sender_currency_rate) + $total_charge;
                $data = [
                    'requested_amount'          => $amount,
                    'fixed_charge'              => $trx_charge->fixed_charge,
                    'percent_charge'            => $top_up_game_percent_charge,
                    'sender_cur_code'           => $currency->currency_code,
                    'sender_cur_rate'           => $sender_currency_rate ?? 0,
                    'total_charge'              => $total_charge,
    ...
    public function responseReceive($type = null)
    {
        ...
        if (empty($tempData) || empty($tempData['type'])) {
            $error = ['error' => [__('Transaction failed. The record did not save properly. Please try again.')]];
            return Helpers::error($error);
        }
        ...
        } elseif ($type == 'uddoktapay') {
            if (method_exists(UddoktaPay::class, $method_name)) {
                return $this->$method_name($this->output);
            }
        } else {
    ...
    public function billingTempData($data)
    {
        $this->output['billingTempData']  = $data;
        return $this;
    }
